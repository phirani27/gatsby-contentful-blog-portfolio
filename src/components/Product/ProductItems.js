      ...
      <ol className={styles.productList}>
            {products.map((product)=>{
                ...
                if(product.masterData.current.variants[0].images.length>0){
                    imageTag = (<img width="200" height="200" alt={product.masterData.current.description} src={product.masterData.current.variants[0].images[0].url}/>);
                }
                return (
                    <li className={styles.product} key={product.key}>
                        <AniLink to={"/"+ product.key.toLowerCase()+"/"+product.masterData.current.slug} >
                            {imageTag}
                            <div>
